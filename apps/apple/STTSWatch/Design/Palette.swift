...
/// this file, and views reference the tokens by name. The watch background is
/// true black. There is one dark appearance; there is no light variant.
///
/// Type roles map onto the system text styles so Dynamic Type keeps working
/// (`largeTitle`, `title2`, `body`, `footnote`, `caption2`). watchOS text
/// styles carry the platform's own metrics, so the rendered sizes match the
/// design scale in intent — hierarchy and contrast — as the design system's
/// watch-materials clause directs.
...
extension Color {
    ...
    /// `surface` — list row, secondary surface.
