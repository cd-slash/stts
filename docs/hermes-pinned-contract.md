...
The diagnostic policies and tokens used for verification were deleted immediately afterward. Production must use a separately named, host-specific service token stored only as Worker secrets. The Worker now implements the bounded bootstrap, authenticated upgrade, first-turn `session.create`, subsequent `session.resume`, prompt submission, and completed-event collection sequence.
