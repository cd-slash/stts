...
The native Apple clients authenticate with an owner-provisioned Access service token held in the iOS Keychain and sent as `CF-Access-Client-Id` / `CF-Access-Client-Secret`. The Worker binds that identity to `service:<common_name>`, so native handles are not interchangeable with the web client's. The watchOS app holds no credential and reaches the Worker only through the phone. Deleting the service token revokes the device, and because handles are bound to the subject fingerprint, its outstanding handles also stop validating. Native clients retain meeting transcripts on device under the app's protected container and delete audio segments after successful transcription.
